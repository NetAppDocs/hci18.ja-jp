...
手順に従ってコントローラアセットを更新します。を参照して link:task_mnode_edit_vcenter_assets.html["既存のコントローラアセットを表示または編集する"]
...
.手順
. 管理ノードのREST APIユーザインターフェイスの概要については、を参照してlink:task_mnode_work_overview_API.html["管理ノードの REST API ユーザインターフェイスの概要"]ください。
...
. REST API UIから、* get ​/assets/compute_nodes *を選択します。
+
...
+
.. PUT /assets/｛asset_id｝/compute-nodes/｛compute_id｝*を選択します。
+
