...
. [Rancher Server Clusters]ペインで、*[Configure Rancher Server Clusters]*を選択します。
. 削除する必要があるランチャのインストールの * アクション * メニューを選択します。
+

CAUTION: [削除]*を選択すると、NetApp HCI管理クラスタ上のRancherがただちに削除されます。
...
.手順
. 管理ノードのIPアドレスに続けて次のように入力し `/k8sdeployer/api/`ます。
+
[listing]
----
...
. [POST/DESTROY]*を選択します。
. [* 試してみてください * ] を選択します。
. [request body]テキストボックスに、値としてRancherサーバのFQDNを入力し `serverURL`ます。
. [* Execute] を選択します。
+
